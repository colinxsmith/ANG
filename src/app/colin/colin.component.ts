...
export class ColinComponent implements OnInit {
  ...
  BAR(cx: number, cy: number) {
    ...
    const squarePie = function (data: number[], rad1: number, rad2: number, ang1: number, ang2: number) {
      ang1 += Math.PI / 2;
      ang2 += Math.PI / 2;
    ...
    const squareData = squarePie(data1, outerRadius, innerRadius, startAngle, endAngle);
    const oldFig = svg.selectAll('quadr').data(squareData).enter()
      ...
      .attr('d', function (d: { path: string, data: number, angle: number, angle0: number }) {
        return arcfunc({
          startAngle: d.angle0 + Math.PI / 2, endAngle: d.angle + Math.PI / 2
    ...
    const newFig = svg.selectAll('quadr').data(squareData).enter()
      ...
      .transition().delay(1000).attr('transform', `translate(${375},${100}),rotate(0)`)
      ;
    const debHere = false;
    if (debHere) {
